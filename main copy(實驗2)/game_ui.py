...
# ----------------------------
# 新增輸家效果相關資源
loser_image_path = os.path.join("hands", "loser.png")
loser_image = pygame.image.load(loser_image_path) if os.path.exists(loser_image_path) else None
loser_sound = load_sound("loser.wav")
...
def get_deal_target(player_index):
    ...
    # 假設使用 dummy list 模擬新增一張牌
    dummy_hand = [None] * (1)  # 模擬只有一張牌的情況
...
class AnimatedCard:
    ...
    def draw(self, surface):
        # 建立一個具透明背景的 Surface
        base = pygame.Surface((CARD_WIDTH, CARD_HEIGHT), pygame.SRCALPHA)
        base = base.convert_alpha()
        base.fill((0, 0, 0, 0))  # 完全透明
...
def setup_bgm():
    ...
    if os.path.exists(bgm_path):
        ...
        pygame.mixer.music.set_volume(0.25)  # 設定音量為 25%
        pygame.mixer.music.play(-1)
# ----------------------------
def show_loser_effects(surface, winner):
    """
    分出勝負後，對非 winner 的玩家顯示 loser.png 圖片，
    並播放 loser.wav 音效三次。
    ...
    # 定義各玩家顯示輸家效果的固定位置
    positions = {
        0: (WIDTH // 2 - 50, HEIGHT - 150),  # bottom
        1: (50, HEIGHT // 2 - 50),            # left
        2: (WIDTH // 2 - 50, 50),             # top
        3: (WIDTH - 150, HEIGHT // 2 - 50)     # right
    }
    ...
    for player in range(4):
        if player != winner and loser_image:
            surface.blit(loser_image, positions[player])

def show_loser_effects(surface, winner):
    """
    顯示所有輸家（非 winner）的 loser.png 圖片，
    並播放 loser.wav 3 次（間隔播放）。
    """
    positions = {
        0: (WIDTH // 2 - 50, HEIGHT - 150),  # bottom
        1: (50, HEIGHT // 2 - 50),           # left
...
def main():
    ...
    pygame.display.set_caption("遊戲示範")
    ...
    while running:
        ...
        draw(screen)
        # 顯示輸家效果
        show_loser_effects(screen, winner)
        pygame.display.flip()
        clock.tick(60)
